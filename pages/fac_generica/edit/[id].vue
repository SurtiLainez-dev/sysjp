      ...
      <v-col cols="12" xl="11">
        <v-card class="rounded-xl elevation-2">
          <v-card-title class="d-flex align-center justify-space-between flex-wrap ga-3 px-6 py-5">
            <div>
              <div class="text-h5 font-weight-bold">Editar Factura Genérica</div>
              <div class="text-medium-emphasis text-body-2">
                Modifica todos los datos de la factura y guarda los cambios.
              </div>
            </div>
            ...
            <div class="d-flex ga-2 flex-wrap">
              <v-btn
                  ...
                  @click="router.push('/fac_generica/index')"
              >
                Regresar
              </v-btn>
          ...
          <v-card-text class="px-6 py-6">
            ...
            <div v-else-if="!cargada" class="py-12 text-center text-medium-emphasis">
            ...
            <template v-else>
              ...
              <div class="d-flex align-center justify-space-between mt-6 mb-3 flex-wrap ga-2">
                <div>
                  ...
                  <div class="text-body-2 text-medium-emphasis">
                    Puedes agregar, quitar y modificar todas las filas.
                  </div>
                </div>
              ...
              <div class="d-flex justify-end mt-6 ga-3 flex-wrap">
                <v-btn
                    ...
                    color="grey-darken-1"
                    :disabled="guardando"
                    @click="router.push(`/fac_generica/${route.params.id}`)"
                >
                  Cancelar
                </v-btn>
...
function formatDateInput(date: string | Date | null | undefined) {
  if (!date) return ""
...
async function cargarFactura() {
  try {
    ...
    form.fecha = formatDateInput(factura.fecha)
    form.cliente = factura.cliente || ""
    form.direccion_envio = factura.direccion_envio || ""
    form.fecha_maximo_envio = formatDateInput(factura.fecha_maximo_envio)
    form.paga_con_tarjeta = Boolean(factura.paga_con_tarjeta)
    form.observacion = factura.observacion || ""
    form.cuerpo = (factura.cuerpo || []).map((item: any) => ({
      uid: `${item.id || Date.now()}-${Math.random().toString(36).slice(2, 9)}`,
      cod: item.cod || "",
      articulo: item.articulo || "",
      precio: num(item.precio),
      cantidad: num(item.cantidad),
      descuento_unidad: num(item.descuento_unidad),
    }))
...
async function actualizarFactura() {
  ...
  try {
    ...
    await $fetch(`/api/fac_generica/${route.params.id}/edit`, {
      method: "PUT",
      body: {
    ...
    setTimeout(() => {
      router.push(`/fac_generica/${route.params.id}`)
    }, 500)
  } catch (error: any) {
    const message =
        error?.data?.statusMessage ||
