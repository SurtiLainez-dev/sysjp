...
function generarNumeroFactura(consecutivo: number): string {
    return String(consecutivo).padStart(10, "0")
}
...
export default defineEventHandler(async (event) => {
    try {
        ...
        const factura = await prisma.facGenerica.create({
            data: {
                numero,
                fecha,
                cliente,
        ...
        console.error("ERROR guardar fac_generica:", error)

        throw createError({
            statusCode: error?.statusCode || 500,
            statusMessage:
                error?.statusMessage || error?.message || "Error al guardar la factura",
        })
    }
})
